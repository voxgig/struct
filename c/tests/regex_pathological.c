/* Discovery test: pathological regex inputs run against the port's voxgig_re_*
...
static void print_strvec(const voxgig_strvec* v) {
  printf("[");
...
int main(void) {
  ...
  /* P4 */
  t0 = now_ms();
  char* p4 = voxgig_re_replace("\\.", "café.au.lait", "/");
  ms = now_ms() - t0;
  ...
  /* P5 */
  t0 = now_ms();
  voxgig_strvec p5 = voxgig_re_find("é", "café au lait");
  ms = now_ms() - t0;
  printf("[regex-discovery] P5_unicode_find_codepoint | %.2fms | OK | ", ms);
  print_strvec(&p5);
  printf("\n");
  voxgig_strvec_free(&p5);

  /* P6 */
  t0 = now_ms();
  bool b6 = voxgig_re_test(nest40, "a");
  ms = now_ms() - t0;
  ...
  bool b7 = voxgig_re_test("^a{0,10000}b$", p7_in);
  ms = now_ms() - t0;
  printf("[regex-discovery] P7_big_bounded_quantifier | %.2fms | OK | %s\n", ms,
         b7 ? "true" : "false");
  free(p7_in);

  /* P8 — invalid pattern. voxgig_re_compile returns NULL on error. */
  t0 = now_ms();
  voxgig_regex* p8 = voxgig_re_compile("[abc");
  ms = now_ms() - t0;
  if (p8) {
    printf("[regex-discovery] P8_invalid_pattern | %.2fms | OK | \"compiled-without-error\"\n", ms);
    /* leak: no voxgig_regex_free in public header */
  } else {
  ...
  /* P9 */
  t0 = now_ms();
  bool b9 = voxgig_re_test("^(a+)\\1$", "aaaa");
  ms = now_ms() - t0;
  ...
  /* P10 */
  t0 = now_ms();
  voxgig_strvec_vec p10 = voxgig_re_find_all("a*", "bbb");
  ms = now_ms() - t0;
