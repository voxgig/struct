...
Port of the canonical TypeScript implementation (`typescript/src/StructUtility.ts`).
...
- **`undefined` vs `null`**: `Value::Noval` vs `Value::Null`, never collapsed and never
  ...
  / `__EXISTS__` marker strings exactly as `typescript/test/runner.ts` does.
...
- The `primary.check` SDK test uses a minimal mock SDK in `tests/corpus.rs`
  (mirroring `typescript/test/sdk.ts`): `check(ctx)` returns
