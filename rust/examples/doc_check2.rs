use voxgig_struct::{transform, validate, walk, select, jsonify, stringify, InjectDef, JsonFlags, Value};
...
fn main() {
    ...
    let _ = select(&children, &Value::map_of([("age".into(), Value::Num(30.0))]));

    // jsonify / stringify
    let _ = jsonify(&data, None);
    let _ = jsonify(&data, Some(&JsonFlags { indent: 0, offset: 0 }));
    let _ = stringify(&data, Some(80), false);

    // custom transform func via extra
    let sum = Value::func(|_inj, val, _ref, _store| val.clone());
    let def2 = InjectDef { extra: Some(Value::map_of([("sum".into(), sum)])), ..Default::default() };
    ...
    let _ = voxgig_struct::get_path(&store, &Value::list(vec![Value::str("a.b"), Value::str("c")]), None);
