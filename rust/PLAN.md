...
> [`typescript/src/StructUtility.ts`](../typescript/src/StructUtility.ts) (~3,135 lines);
...
1. **Drop the pool; use push/pop backtracking.** `walk_impl(val,
   ...
   (There's a `walk-bench` test in `typescript/test/`, so this is on the
...
- The corpus runner matches expected errors by substring/regex against
  `err.message` (see `typescript/test/runner.ts` `handleError`/`matchval`), so
...
Also: add `rust/target/` (and `rust/Cargo.lock` policy — commit it, it's a
...
| 11 | Corpus runner deep-equality + marker semantics (`__NULL__`/`__UNDEF__`/`__EXISTS__`) and `/regex/` `match` entries | Getting the comparator wrong = false reds/greens. | Port `typescript/test/runner.ts` carefully; map `deepStrictEqual` to an order-independent deep compare over `Value`; reuse the canonical's own `walk`/`getpath`/`stringify` in the `match` path exactly as the TS runner does. |
