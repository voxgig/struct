...
using Injector = std::function<Value(Injection& inj, const Value& val, const std::string& ref,
                                     const Value& store)>;

// Modify: optional value-mutation hook used by inject/transform/validate.
// Mirrors TS `Modify`.
using Modify = std::function<void(const Value& val, const Value& key, const Value& parent,
...
class Value {
public:
  using Storage = std::variant<std::monostate,        // 0  - undefined / T_noval
  ...
  double as_double() const {
    ...
    if (is_int())
      return static_cast<double>(std::get<int64_t>(storage));
    return 0.0;
  }
...
inline Value SKIP() {
  return Value(&skip_inst());
}
inline Value DELETE_V() {
  return Value(&delete_inst());
}
  ...
  bool operator==(const OrderedMap& other) const {
    ...
    for (size_t i = 0; i < entries_.size(); i++) {
      if (entries_[i].first != other.entries_[i].first)
        return false;
      if (entries_[i].second != other.entries_[i].second)
        return false;
    }
    return true;
  }
...
inline bool operator==(const Value& a, const Value& b) {
  ...
  if (a.is_sentinel() || b.is_sentinel()) {
    return a.is_sentinel() && b.is_sentinel() && a.as_sentinel() == b.as_sentinel();
  }
  ...
  if (a.is_list()) {
    ...
    auto lb = b.as_list();
    if (la == lb)
      return true; // same shared instance
    if (!la || !lb)
      return false;
    if (la->size() != lb->size())
      return false;
...
inline int typify(const Value& v) {
  ...
  if (v.is_sentinel())
    return T_node | T_map; // SKIP/DELETE behave as maps
  return T_any;
}
...
inline Value clone(const Value& v) {
  if (v.is_sentinel())
    return v; // preserve identity
  if (v.is_list())
    return Value(clone_list(v.as_list()));
  if (v.is_map())
    return Value(clone_map(v.as_map()));
...
inline int64_t size_of(const Value& v) {
  if (v.is_list())
    return static_cast<int64_t>(v.as_list()->size());
  if (v.is_map())
    return static_cast<int64_t>(v.as_map()->size());
  if (v.is_string())
    return static_cast<int64_t>(v.as_string().size());
  if (v.is_int())
    return static_cast<int64_t>(std::floor(static_cast<double>(v.as_int())));
  if (v.is_double())
    return static_cast<int64_t>(std::floor(v.as_double()));
  if (v.is_bool())
