...
inline bool parse_string(State& p, std::string& out) {
  ...
  while (p.pos < p.src.size()) {
    ...
    if (c == '"') return true;
    if (c == '\\') {
      if (p.pos >= p.src.size()) return false;
      char e = p.src[p.pos++];
      switch (e) {
        ...
        case 'u': {
          if (p.pos + 4 > p.src.size()) return false;
          ...
          p.pos += 4;
          if (cp >= 0xD800 && cp <= 0xDBFF && p.pos + 6 <= p.src.size()
...
inline Value parse_array(State& p) {
  ...
  skip_ws(p);
  if (peek(p) == ']') { p.pos++; return Value(std::move(out)); }
  while (true) {
    skip_ws(p);
...
inline Value parse_value(State& p) {
  ...
  int c = peek(p);
  if (c < 0) return Value();
  if (c == 'n' && match(p, "null")) return Value(nullptr);
  if (c == 't' && match(p, "true")) return Value(true);
  if (c == 'f' && match(p, "false")) return Value(false);
  if (c == '"') {
    std::string s;
    parse_string(p, s);
