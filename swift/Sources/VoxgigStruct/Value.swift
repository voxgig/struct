...
public indirect enum Value: @unchecked Sendable {
  ...
  public static func map(_ entries: OrderedDictionary<String, Value>) -> Value {
    .map(VMap(entries))
  }
  ...
  public var asFunction: Injector? {
    if case .function(let f) = self { return f } else { return nil }
  }
  public var asSentinel: Sentinel? {
    if case .sentinel(let s) = self { return s } else { return nil }
  }
