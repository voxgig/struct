...
fileprivate func invalidTypeMsg(_ path: [String], _ needtype: String, _ vt: Int, _ v: Value, _ whence: String) -> String {
    let vs = v.isNoval ? "no value" : stringify(v)
    let field = path.count > 1 ? "field " + pathify(.list(VList(path.map { Value.string($0) })), 1) + " to be " : ""
...
public func validate_STRING(_ inj: Injection, _ val: Value, _ ref: String, _ store: Value) -> Value {
    ...
    if case .string(let s) = out, s.isEmpty {
        inj.errs.items.append(.string("Empty string at " + pathify(.list(VList(inj.path.map { Value.string($0) })), 1)))
        return .noval
    }
    return out
}

fileprivate let TNAME_TO_BIT: [String: Int] = [
    S_nil:      T_noval,
    S_boolean:  T_boolean,
    S_decimal:  T_decimal,
    S_integer:  T_integer,
...
public func validate_CHILD(_ inj: Injection, _ val: Value, _ ref: String, _ store: Value) -> Value {
    ...
    if mode == M_KEYPRE {
        ...
        if tval.isNoval { tval = .map(VMap()) }
        else if !tval.isMap {
            inj.errs.items.append(.string(invalidTypeMsg(
    ...
    if mode == M_VAL {
        ...
        if !inj.dparent.isList {
            let msg = invalidTypeMsg(Array(inj.path.suffix(1)),
...
public func validate_EXACT(_ inj: Injection, _ val: Value, _ ref: String, _ store: Value) -> Value {
    ...
    if mode == M_VAL {
        ...
        var valdesc = tl.items.map { stringify($0) }.joined(separator: ", ")
        valdesc = valdesc.replacingOccurrences(
            of: "`\\$([A-Z]+)`", with: "$1", options: .regularExpression).lowercased()
        inj.errs.items.append(.string(invalidTypeMsg(
            inj.path,
...
public func _validation(_ pval: Value, _ key: Value, _ parent: Value, _ inj: Injection, _ store: Value) {
    ...
    if cval.isMap {
        if !pval.isMap {
            inj.errs.items.append(.string(invalidTypeMsg(inj.path, typename(ptype), ctype, cval, "V0020")))
            return
        }
        ...
        if !pval.isList {
            inj.errs.items.append(.string(invalidTypeMsg(inj.path, typename(ptype), ctype, cval, "V0030")))
        }
    } else if exact {
        if cval != pval {
            let pathmsg = inj.path.count > 1
                ? "at field " + pathify(.list(VList(inj.path.map { Value.string($0) })), 1) + S_VIZ
                : S_MT
            inj.errs.items.append(.string("Value " + pathmsg + stringify(cval) + " should equal " + stringify(pval) + S_DT))
        }
    } else {
