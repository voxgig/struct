...
public func getpath(_ store: Value, _ pathIn: Value, _ inj: Injection? = nil) -> Value {
    ...
    case .string(let s):
        if s.isEmpty { parts = [""] }
        else { parts = s.split(separator: ".", omittingEmptySubsequences: false).map(String.init) }
...
fileprivate func isfunc(_ : Injector) -> Bool { true }
