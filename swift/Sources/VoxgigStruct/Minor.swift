...
@discardableResult
public func slice(_ val: Value, _ start: Int? = nil, _ end: Int? = nil, mutate: Bool = false) -> Value {
    // Numeric input: clamp.
...
public func stringify(_ v: Value, _ maxlen: Int? = nil) -> String {
    ...
    if let m = maxlen, m > 0, s.count > m {
        if m < 3 { s = String(s.prefix(m)) } else {
...
fileprivate func emitSorted(_ v: Value) -> String {
    switch v {
    case .list(let l):
    ...
    case .map(let m):
        ...
        return "{" + keys.map { JSON.quotedKey($0) + ":" + emitSorted(m.entries[$0]!) }.joined(separator: ",") + "}"
...
fileprivate func cloneInner(_ v: Value, _ seen: inout [ObjectIdentifier: Value]) -> Value {
    switch v {
    case .list(let l):
