    ...
    if case .list(let tl) = terms {
        for term in tl.items {
            ...
            if !terrs.items.isEmpty {
                inj.errs.items.append(.string("AND:" + pathify(ppath) + S_VIZ + stringify(point) +
...
public func select_OR(_ inj: Injection, _ val: Value, _ ref: String, _ store: Value) -> Value {
    ...
    inj.errs.items.append(.string("OR:" + pathify(ppath) + S_VIZ + stringify(point) +
        " fail:" + stringify(terms)))
    return .noval
}
...
public func select_NOT(_ inj: Injection, _ val: Value, _ ref: String, _ store: Value) -> Value {
    ...
    _ = validate(point, term, subInj)
    if terrs.items.isEmpty {
        inj.errs.items.append(.string("NOT:" + pathify(ppath) + S_VIZ + stringify(point) +
            " fail:" + stringify(term)))
    }
    let gkey = getelem(.list(VList(inj.path.map { Value.string($0) })), .int(-2))
    let gp = inj.nodes.count >= 2 ? inj.nodes[inj.nodes.count - 2] : .noval
...
public func select_CMP(_ inj: Injection, _ val: Value, _ ref: String, _ store: Value) -> Value {
    ...
    if ref == "$GT" {
        if bothNum, let a = point.asDouble, let b = term.asDouble { pass = a > b }
        else if case .string(let a) = point, case .string(let b) = term { pass = a > b }
    } else if ref == "$LT" {
        if bothNum, let a = point.asDouble, let b = term.asDouble { pass = a < b }
        else if case .string(let a) = point, case .string(let b) = term { pass = a < b }
    } else if ref == "$GTE" {
    ...
    if pass {
        ...
        inj.errs.items.append(.string("CMP: " + pathify(ppath) + S_VIZ + stringify(point) +
