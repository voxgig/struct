...
func nullModifier(_ val: Value, _ key: Value, _ parent: Value, _ inj: Injection, _ store: Value) {
    if case .string(let s) = val {
        if s == NULLMARK { setprop(parent, key, .null) }
...
fileprivate func normaliseAbsent(_ v: Value) -> Value {
    switch v {
...
final class CorpusTests: XCTestCase {
    ...
    private func runset(_ label: String, _ entries: Value, _ subject: (Value) -> Value) {
        ...
        for (i, entry) in l.items.enumerated() {
            ...
            let gotS = canon(got)
            if exp == gotS { pass += 1 }
            else { fails.append("[\(label)#\(i)] exp=\(exp) got=\(gotS)") }
        }
        if !fails.isEmpty {
    ...
    func testMinor() {
        ...
        guard case .map(let minor) = getprop(s, .string("minor")) else { return XCTFail("missing minor") }

        runset("minor.isnode",  getprop(.map(minor), .string("isnode")) |> setProp("set"),
               { .bool(isnode($0)) })
        runset("minor.ismap",   getprop(.map(minor), .string("ismap")) |> setProp("set"),
               { .bool(ismap($0)) })
        runset("minor.islist",  getprop(.map(minor), .string("islist")) |> setProp("set"),
               { .bool(islist($0)) })
        runset("minor.iskey",   getprop(.map(minor), .string("iskey")) |> setProp("set"),
               { .bool(iskey($0)) })
        runset("minor.isempty", getprop(.map(minor), .string("isempty")) |> setProp("set"),
               { .bool(isempty($0)) })
        runset("minor.size",    getprop(.map(minor), .string("size")) |> setProp("set"),
               { .int(Int64(size($0))) })
        runset("minor.keysof",  getprop(.map(minor), .string("keysof")) |> setProp("set"),
               { .list(keysof($0).map { Value.string($0) }) })
        runset("minor.haskey",  getprop(.map(minor), .string("haskey")) |> setProp("set"),
               { .bool(haskey(getprop($0, .string("src")), getprop($0, .string("key")))) })
        runset("minor.getprop", getprop(.map(minor), .string("getprop")) |> setProp("set"),
               { getprop(getprop($0, .string("val")),
                         getprop($0, .string("key")),
                         getprop($0, .string("alt"))) })
        runset("minor.clone",   getprop(.map(minor), .string("clone")) |> setProp("set"),
               { clone($0) })
        runset("minor.escre",   getprop(.map(minor), .string("escre")) |> setProp("set"),
               { .string(escre($0)) })
        runset("minor.escurl",  getprop(.map(minor), .string("escurl")) |> setProp("set"),
               { .string(escurl($0)) })
        runset("minor.stringify", getprop(.map(minor), .string("stringify")) |> setProp("set"),
               {
                   if case .map(let m) = $0 {
    ...
    func testInject() {
        ...
        if let basic = im.entries["basic"], case .map(let bm) = basic, let inv = bm.entries["in"] {
            let got = inject(clone(getprop(inv, .string("val"))),
        ...
        if let stringSet = im.entries["string"], case .map(let sm) = stringSet,
           let set = sm.entries["set"], case .list(let sl) = set {
            ...
            for (i, entry) in sl.items.enumerated() {
                ...
                if canon(got) == canon(exp) { pass += 1 }
                else { fail += 1; if fail <= 5 { print("[inject.string#\(i)] exp=\(canon(exp)) got=\(canon(got))") } }
            }
            if fail > 0 { XCTFail("inject.string: \(pass)/\(pass + fail)") }
            else { print("ok inject.string: \(pass)/\(pass)") }
        }
        if let deepSet = im.entries["deep"], case .map(let dm) = deepSet,
           let set = dm.entries["set"] {
            runset("inject.deep", set, {
                inject(clone(getprop($0, .string("val"))),
    ...
    func testTransform() {
        ...
        if let basic = tm.entries["basic"], case .map(let bm) = basic, let inv = bm.entries["in"] {
            let got = transform(clone(getprop(inv, .string("data"))),
        ...
        for name in ["paths", "cmds", "each", "pack", "ref", "apply", "format"] {
            if let sub = tm.entries[name], case .map(let sm) = sub,
               let set = sm.entries["set"] {
                runset("transform.\(name)", set, {
                    transform(clone(getprop($0, .string("data"))),
        ...
        if let sub = tm.entries["modify"], case .map(let sm) = sub,
           let set = sm.entries["set"] {
            runset("transform.modify", set, {
                ...
                return transform(clone(getprop($0, .string("data"))),
                                 clone(getprop($0, .string("spec"))),
                                 runInj)
            })
        }
    ...
    func testValidate() {
        ...
        for name in ["basic", "child", "one", "exact", "invalid", "special"] {
            if let sub = vm.entries[name], case .map(let sm) = sub,
               let set = sm.entries["set"] {
                runset("validate.\(name)", set, { entry in
                    ...
                    return validate(clone(getprop(entry, .string("data"))),
            ...
            if let sub = sm.entries[name], case .map(let ssm) = sub,
               let set = ssm.entries["set"], case .list(let sl) = set {
                ...
                for (i, entry) in sl.items.enumerated() {
                    ...
                    if canon(got) == canon(exp) { pass += 1 }
                    else { fail += 1; if fail <= 5 {
                        print("[select.\(name)#\(i)] exp=\(canon(exp)) got=\(canon(got))")
                    } }
                }
                if fail > 0 { XCTFail("select.\(name): \(pass)/\(pass + fail)") }
                else { print("ok select.\(name): \(pass)/\(pass)") }
            }
        }
    }
}
...
fileprivate func |> (lhs: Value, rhs: (Value) -> Value) -> Value { rhs(lhs) }
fileprivate func setProp(_ key: String) -> (Value) -> Value {
