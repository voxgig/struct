...
`.github/workflows/build.yml` runs only on a manual dispatch, so nothing
gates the port against the engine commit that workflow pins on a push or a
pull request. Dispatching the workflow by hand still runs it.
