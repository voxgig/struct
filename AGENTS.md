...
- **It reads only `<lang>/v*` tags.** `_add_tag` matches `^([a-z+]+)/v(.+)$`
  ...
  0.3.4 was live. `publish.yml` writes `typescript/v<version>` now and
  `typescript/v0.3.4` sits beside the bare tag, so the port reads correctly
  from here on; the bare tags stay as history.
