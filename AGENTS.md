...
| Scala | `scala/` | `make test` (`scala-cli`) | compile check (`scala-cli compile`) | Scala 3, `scala-cli` only — no `scalac`/`scala`; `Value` ADT; distinct Noval/VNull (like TS); `java.util.regex` |
