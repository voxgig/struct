...
shared `build/test/*.aon` corpus is the contract.** This port follows
...
**That job is held inactive as of 2026-09-04** — it carries `if: false`,
and the port is out of the root Makefile's aggregate targets as well. So
the protection this section describes is not currently in force, and the
silent breakage above can recur. Deleting the `if: false` restores it.
