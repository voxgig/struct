...
function measure(label, tree, runs) {
    ...
    // Warm-up.
    for (let i = 0; i < 2; i++) {
        (0, StructUtility_1.walk)(tree, cb);
    }
    const times = [];
    ...
    console.log(`[walk-bench] ${label}: nodes=${nodes} runs=${runs} ` +
...
(0, node_test_1.describe)('walk-bench', () => {
    (0, node_test_1.test)('walk-bench-wide-and-deep', { skip: !BENCH }, () => {
        // ~299k nodes: width=8, depth=6.
        const wideDeep = buildTree(8, 6);
        measure('wide+deep (w=8,d=6)', wideDeep, 7);
    });
    (0, node_test_1.test)('walk-bench-very-wide', { skip: !BENCH }, () => {
        // Single root with many shallow children: width=10000, depth=2 -> ~100M? too big.
        // Use width=5000, depth=2 -> 5000*5000+5000+1 = 25,005,001. Still big.
        // Use width=500, depth=3 -> 500^3+500^2+500+1 = 125,250,501. Too big.
        // Use width=1000, depth=2 -> 1,001,001. ~1M nodes, shallow.
...
//# sourceMappingURL=walk-bench.test.js.map